...
#| fig-cap: "Map of biomass prediction (tDM/ha) for Douglas Fir in Malcom Knapp in 2100."
...
#| fig-cap: "Map of biomass prediction (tDM/ha) for Douglas Fir in Malcom Knapp in 2024."
...
Note how the results we obtained closely mirrors the predicted biomass in 2100 we estimated earlier. This results shows how dependent the model is on the input data. By giving it the same climate dataset, which is drawn by climate projection based on SSP2 (@tbl-sources), the output we obtain is pretty similar except for the missing values where we don't have age information, and the darker areas that have probably been harvested recently.
...
# add volume info
tally <- tally %>% 
  mutate(volume = (1/3) * pi * ((dbh / 2) ^ 2) * height)
...
To compare the values we got in the field with the pixel-level biomass content, we need to georeference the plot centers, that is assign geospatial information to each plot in order to properly place them on the map. We did this in the field, so let's add this information to our current dataframe (@fig-plots).
...
To finalize the comparison, let's take the pixels corresponding to the location of our plots. We can do this by using the geospatial information attached to every plot.
