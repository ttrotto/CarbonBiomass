...
by_plot <- tally %>%
  group_by(plot) %>%
  summarize(plot_biomass = mean(volume))
by_plot
```
...
extracted_biomass <- terra::extract(r,
                                    vect(by_plot),
                                    touches = T) %>%
  rename(plot = ID,
         model_biomass = z)
...
# comparison
comparison <- left_join(extracted_biomass, st_drop_geometry(by_plot), by = 'plot')
comparison
```
